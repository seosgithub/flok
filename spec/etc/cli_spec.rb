...
#it "Can create a new project with correct directories" do
  ...
  it "Can build a project that contains a transition" do
    #Compile and then return the length of the application_user.js file
    def compile_with_files controller_path=nil, transition_path=nil
      #Custom controller to test source with
      controller_src = File.read(controller_path) if controller_path
      transition_src = File.read(transition_path) if transition_path
      flok_new do
        File.write "./app/controllers/controller1.rb", controller_src if controller_path
        File.write "./app/transitions/transition1.rb", transition_src if transition_path
    ...
    #The file with the transition should be longer than the one without the transition
    len_with_content = compile_with_files "./spec/etc/user_compiler/controller1.rb", "./spec/etc/transition_compiler/transition1.rb"
    len_no_content = compile_with_files "./spec/etc/user_compiler/controller1.rb" 
